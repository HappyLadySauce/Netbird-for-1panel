...
| 仓库路径 | 复制到 1Panel（宿主机） |
|----------|-------------------------|
| [docs/proxy/netbird-server.conf](proxy/netbird-server.conf) | `/opt/1panel/www/sites/<你的域名>/proxy/netbird-server.conf`（**新建**） |
| [docs/proxy/root.conf](proxy/root.conf) | `/opt/1panel/www/sites/<你的域名>/proxy/root.conf`（**覆盖**） |

详细说明与一键命令见：[docs/proxy/README.md](proxy/README.md)。
...
若安装表单使用了非默认端口，请先改 [docs/proxy/](proxy/) 内文件中的 `8080` / `8081`，再复制覆盖。
...
即：**新建** `netbird-server.conf`，**覆盖** `root.conf`，内容与仓库 [docs/proxy/](proxy/) 一致即可生效。
...
| **使用 [docs/proxy/](proxy/) 覆盖** | 与官方外部反代要求一致，**推荐** |

安装应用后生成的 `data/openresty-snippet.conf` 仅作参考；**以本仓库 `docs/proxy/` 为准**。
